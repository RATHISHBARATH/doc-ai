# ============================================================
# DOC AI Vision Service – MinIO Client
# ============================================================
...
class MinIOClient:
    ...
    def __init__(self, config=None):
        ...
        if config is None:
            config = get_config()
        self.config = config
        self.bucket = config.storage.minio_bucket

        # Create the MinIO client
        self.client = Minio(
            endpoint=os.environ.get("VISION_MINIO_ENDPOINT", "minio:9000"),
            access_key=os.environ.get("VISION_MINIO_ACCESS_KEY", "minioadmin"),
            secret_key=os.environ.get("VISION_MINIO_SECRET_KEY", "changeme"),
            secure=False,
        )
    ...
    def upload_bytes(self, data: bytes, remote_path: str, content_type: str = "application/octet-stream") -> ObjectWriteResult:
        ...
        Useful for small objects (e.g., metadata, configuration files).
        """
        try:
            stream = BytesIO(data)
